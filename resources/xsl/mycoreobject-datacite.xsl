...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="publicationYear">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="//service/servdates/servdate[@type='createdate']" mode="publicationYear" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="mods:dateCreated|mods:dateIssued|servdate" mode="publicationYear">
    <publicationYear>
  ...
  <xsl:template name="resourceType">
    <resourceType resourceTypeGeneral="Text">
      <xsl:attribute name="resourceTypeGeneral">
        <xsl:choose>
          <xsl:when test="substring-after(mods:genre/@valueURI, '#') = 'video'">
            <xsl:text>Audiovisual</xsl:text>
          </xsl:when>
          <xsl:when test="substring-after(mods:genre/@valueURI, '#') = 'audio'">
